...
var planet_scene = {};
planet_scene.region = {};
planet_scene.init = function (data) {
  generationSettings.seed = data.seed;
  generationSettings.subdivisions = data.detail_level;
  ...
  //$add player
  ui.surfaceDisplayButtons.player.click(setSurfaceRenderMode.bind(null, 'player'));
  //
  ...
  $('#filter_day_night').on('change', function (e) {
    showHideSunlight();
  });
  $('#filter_plate_border').on('change', function (e) {
    showHidePlateBoundaries();
  });
  $('#filter_plate_move').on('change', function (e) {
    showHidePlateMovements();
  });
  $('#filter_wind').on('change', function (e) {
    showHideAirCurrents();
  });
  $('#filter_map_player').on('change', function (e) {
    if ($(this).prop('checked')) {
      $('#map_type input').not(this).prop('checked', false).switchButton({ checked: false });
      setSurfaceRenderMode('player');
    } else setSurfaceRenderMode('terrain');
  });
  $('#filter_map_elevation').on('change', function (e) {
    if ($(this).prop('checked')) {
      $('#map_type input').not(this).prop('checked', false).switchButton({ checked: false });
      setSurfaceRenderMode('elevation');
    } else setSurfaceRenderMode('terrain');
  });
  $('#filter_map_plate').on('change', function (e) {
    if ($(this).prop('checked')) {
      $('#map_type input').not(this).prop('checked', false).switchButton({ checked: false });
      setSurfaceRenderMode('plates');
    } else setSurfaceRenderMode('terrain');
  });
  $('#filter_map_temperature').on('change', function (e) {
    if ($(this).prop('checked')) {
      $('#map_type input').not(this).prop('checked', false).switchButton({ checked: false });
      setSurfaceRenderMode('temperature');
    } else setSurfaceRenderMode('terrain');
  });
  $('#filter_map_moisture').on('change', function (e) {
    if ($(this).prop('checked')) {
      $('#map_type input').not(this).prop('checked', false).switchButton({ checked: false });
  ...
  //mercatorMap
  //globe
  $('#filter_projection_globe').on('change', function (e) {
    if ($(this).prop('checked')) {
      $('#projection_type input').not(this).prop('checked', false).switchButton({ checked: false });
      setProjectionRenderMode('globe');
    } else $(this).click();
    //setProjectionRenderMode("globe");
    //$(this).prop('checked', true).switchButton({'checked': true});
  });
  $('#filter_projection_equal').on('change', function (e) {
    if ($(this).prop('checked')) {
      $('#projection_type input').not(this).prop('checked', false).switchButton({ checked: false });
      setProjectionRenderMode('equalAreaMap');
    } else setProjectionRenderMode('globe');
  });
  $('#filter_projection_mercator').on('change', function (e) {
    if ($(this).prop('checked')) {
      $('#projection_type input').not(this).prop('checked', false).switchButton({ checked: false });
  ...
  ui.tectonicPlateCountRange.on('input', function () {
    setPlateCount(
      Math.floor(
        Math.pow(2, (parseInt(ui.tectonicPlateCountRange.val()) / 300) * (Math.log(1000) / Math.log(2) - 1) + 1)
      )
    );
  });
...
//$add
planet_scene.regionInit = function (data) {
  this.region = data;
...
function setPlateCount(plateCount) {
  if (typeof plateCount === 'number' && plateCount >= 0) {
    ...
    var sliderVal = Math.ceil(
      ((Math.log(plateCount) / Math.log(2) - 1) / (Math.log(1000) / Math.log(2) - 1)) * 300
    ).toFixed(0);
...
function generatePlanetAsynchronous() {
  ...
  if (generationSettings.distortionLevel < 0.25)
    distortionRate = adjustRange(generationSettings.distortionLevel, 0.0, 0.25, 0.0, 0.04);
  else if (generationSettings.distortionLevel < 0.5)
    distortionRate = adjustRange(generationSettings.distortionLevel, 0.25, 0.5, 0.04, 0.05);
  else if (generationSettings.distortionLevel < 0.75)
    distortionRate = adjustRange(generationSettings.distortionLevel, 0.5, 0.75, 0.05, 0.075);
  else distortionRate = adjustRange(generationSettings.distortionLevel, 0.75, 1.0, 0.075, 0.15);
...
function generateIcosahedron() {
  ...
  for (var i = 0; i < edges.length; ++i) for (var j = 0; j < edges[i].n.length; ++j) nodes[j].e.push(i);

  for (var i = 0; i < faces.length; ++i) for (var j = 0; j < faces[i].n.length; ++j) nodes[j].f.push(i);

  for (var i = 0; i < faces.length; ++i) for (var j = 0; j < faces[i].e.length; ++j) edges[j].f.push(i);
...
function generateSubdividedIcosahedron(degree) {
  ...
  for (var i = 0; i < icosahedron.faces.length; ++i) {
    ...
    for (var j = 0; j < edge0.subdivided_n.length; ++j) faceNodes.push(getEdgeNode0(j));
...
function relaxMesh(mesh, multiplier, action) {
  ...
  action.executeSubaction(function (action) {
    for (var i = 0; i < mesh.nodes.length; ++i) pointShifts[i] = new Vector3(0, 0, 0);
  }, 1);
...
function blurPlateBoundaryStress(boundaryCorners, stressBlurIterations, stressBlurCenterWeighting, action) {
  ...
  for (var i = 0; i < stressBlurIterations; ++i) {
    for (var j = 0; j < boundaryCorners.length; ++j) {
      ...
      newCornerShear[j] =
        corner.shear * stressBlurCenterWeighting + (averageShear / neighborCount) * (1 - stressBlurCenterWeighting);
    }
  ...
  for (var i = 0; i < boundaryCorners.length; ++i) {
    ...
    if (innerBorderIndex !== null) {
      ...
      if (corner.pressure > 0.3) {
        ...
        if (plate0.oceanic === plate1.oceanic) calculateElevation = calculateCollidingElevation;
        else if (plate0.oceanic) calculateElevation = calculateSubductingElevation;
...
function initializeAirMoisture(corners, moistureLevel, action) {
  ...
  for (var i = 0; i < corners.length; ++i) {
    var corner = corners[i];
    corner.airMoisture =
      corner.elevation > 0 ? 0 : corner.area * moistureLevel * Math.max(0, Math.min(0.5 + corner.temperature * 0.5, 1));
...
function buildSurfaceRenderObject(tiles, random, action) {
  ...
  action.executeSubaction(function (action) {
    ...
    for (var index = 0; index < planet_scene.region.length; index++) {
      if (planet_scene.region[index].region_id == tile.id) {
        if (tile.elevation <= 0) {
          //water color
          playerColor = new THREE.Color('#' + planet_scene.region[index].User.color).lerp(
            new THREE.Color('white'),
            0.5
          );
        } else {
          //land color
          playerColor = new THREE.Color('#' + planet_scene.region[index].User.color); //.lerp(new THREE.Color(elevationColor), 0.7);
        }
        break;
      } else {
    ...
    for (var j = 0; j < tile.corners.length; ++j) {
      ...
      //$add player
      buildTileWedgeColors(playerColors, playerColor, playerColor.clone().multiplyScalar(0.5));
      //
...
function buildPlateBoundariesRenderObject(borders, action) {
  ...
  action.executeSubaction(function (action) {
    ...
    if (border.betweenPlates) {
      ...
      geometry.faces.push(
        new THREE.Face3(baseIndex + 0, baseIndex + 1, baseIndex + 2, normal, [innerColor, innerColor, outerColor])
      );
      geometry.faces.push(
        new THREE.Face3(baseIndex + 1, baseIndex + 3, baseIndex + 2, normal, [innerColor, outerColor, outerColor])
      );
      geometry.faces.push(
        new THREE.Face3(baseIndex + 1, baseIndex + 0, baseIndex + 5, normal, [innerColor, innerColor, outerColor])
      );
      geometry.faces.push(
        new THREE.Face3(baseIndex + 0, baseIndex + 4, baseIndex + 5, normal, [innerColor, outerColor, outerColor])
      );
    }
  ...
  while (Date.now() < this.intervalEndTime && !this.canceled && !this.completed) {
    ...
    if (this.subactions.length > 0) {
      this.beginSubactions(
        ...
        this.callStack.loop
          ? 0
          : (((1 - this.callStack.loopProgress) * action.proportion) / this.callStack.proportionSum) *
...
SteppedAction.prototype.getProgress = function SteppedAction_getProgress() {
  if (this.callStack !== null) {
    ...
    return (
      ...
      (currentProportionSum / this.callStack.proportionSum) * this.callStack.parentProgressRange
    );
  } else {
...
function render() {
  ...
  var frameDuration =
    lastRenderFrameTime !== null ? Math.min((currentRenderFrameTime - lastRenderFrameTime) * 0.001, 0.1) : 0;
  ...
  if (frameDuration > 0 && cameraLongitudeDelta !== 0) {
    cameraLongitude +=
      ...
      Math.PI *
      ((zoom * Math.PI) / 8 + (1 - zoom) / (20 * Math.max(Math.cos(cameraLatitude), 0.1)));
...
var mapProjections = {
  ...
  mercatorMap: function mercatorMap(longitude, latitude, z) {
    return new Vector3(
      longitude / Math.PI,
      Math.log(Math.tan(Math.PI / 4 + Math.max(-1.5, Math.min(latitude * 0.4, 1.5)))) / 1.75,
      z
    );
  },
};
...
function selectTile(tile) {
  ...
  for (var i = 0; i < tile.corners.length; ++i) {
    ...
    geometry.faces.push(
      new THREE.Face3(i + 1, ((i + 1) % tile.corners.length) + 1, 0, tile.normal, [outerColor, outerColor, innerColor])
    );
  }
...
function updateUI() {
  //$ui statistic
  $('.global_planet_name').html('KARA');
  $('#statistic_province_count').html(planet.statistics.tiles.count.toFixed(0));
  $('#statistic_land_percentage').html(
    (
      100 -
      ((planet.statistics.tiles.biomeAreas['ocean'] + planet.statistics.tiles.biomeAreas['oceanGlacier']) /
        planet.statistics.tiles.totalArea) *
        100
    ).toFixed(0) + '%'
  );
  $('#statistic_plate_count').html(planet.statistics.plates.count.toFixed(0));
...
function projectMap(renderData, globeColorArrayKeys, project) {
  ...
  if (globeColorArrayKeys) {
    ...
    for (var i = 0; i < globeGeometry.faces.length; ++i) {
      ...
      mapGeometry.faces.push(
        new THREE.Face3(globeFace.a, globeFace.b, globeFace.c, globeFace.normal, globeFace.vertexColors)
      );
    }
  }
  ...
  if (globeColorArrayKeys) {
    addFace = function addFace(faceIndex, vertexIndex0, vertexIndex1, vertexIndex2) {
      mapGeometry.faces.push(
        new THREE.Face3(vertexIndex0, vertexIndex1, vertexIndex2, globeGeometry.faces[faceIndex].normal)
      );
      for (var i = 0; i < globeColorArrayKeys.length; ++i) {
  ...
  for (var i = 0; i < globeGeometry.faces.length; ++i) {
    ...
    if (xWrap !== 0) {
      if (yWrap !== 0) {
        ...
        if (xWrap01 !== 0) {
          if (xWrap02 !== 0) {
            addFace(i, face.a, addVertex(p1.x - xWrap, p1.y, p1.z), addVertex(p2.x - xWrap, p2.y, p2.z));
            face.a = addVertex(p0.x + xWrap, p0.y, p0.z);
          } else {
            addFace(i, addVertex(p0.x + xWrap, p0.y, p0.z), face.b, addVertex(p2.x + xWrap, p2.y, p2.z));
            face.b = addVertex(p1.x - xWrap, p1.y, p1.z);
          }
        } else if (xWrap02 !== 0) {
          addFace(i, addVertex(p0.x + xWrap, p0.y, p0.z), addVertex(p1.x + xWrap, p1.y, p1.z), face.c);
      ...
      if (yWrap01 !== 0) {
        if (yWrap02 !== 0) {
          addFace(i, face.a, addVertex(p1.x, p1.y - yWrap, p1.z), addVertex(p2.x, p2.y - yWrap, p2.z));
          face.a = addVertex(p0.x, p0.y + yWrap, p0.z);
        } else {
          addFace(i, addVertex(p0.x, p0.y + yWrap, p0.z), face.b, addVertex(p2.x, p2.y + yWrap, p2.z));
          face.b = addVertex(p1.x, p1.y - yWrap, p1.z);
        }
      } else if (yWrap02 !== 0) {
        addFace(i, addVertex(p0.x, p0.y + yWrap, p0.z), addVertex(p1.x, p1.y + yWrap, p1.z), face.c);
...
function setSurfaceRenderMode(mode, force) {
  if (mode !== surfaceRenderMode || force === true) {
    ...
    if (projectionRenderMode === 'globe') {
      ...
      //$add player
      else if (mode === 'player') colors = planet.renderData.surface.playerColors;
      //
      else return;
      //console.log(colors);
    } else {
      ...
      //$add player
      else if (mode === 'player') colors = planet.renderData.surface.mapColorArrays.playerColors;
      //
      else return;
    }
          ...
          fileEntry.createWriter(
            function (fileWriter) {
              fileWriter.addEventListener(
                ...
                function () {
                  $('body').append(
                    '<a href="' + fileEntry.toURL() + '" download="planetMesh.js" target="_blank">Mesh Data</a>'
                  );
                  $('body>a').focus();
                },
                false
