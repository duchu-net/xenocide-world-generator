...
// @formatter:on
export function greekLetterNameToLetter(name) {
    return GREEK_LETTERS[GREEK_LETTERS_NAMES.indexOf(name)];
}
export const numberToGreekChar = (num) => {
...
export const decimalToRoman = (num) => {
    ...
    if (num < 1)
        return '';
    if (num >= 1000)
        return `M${decimalToRoman(num - 1000)}`;
    if (num >= 900)
        return `CM${decimalToRoman(num - 900)}`;
    if (num >= 500)
        return `D${decimalToRoman(num - 500)}`;
    if (num >= 400)
        return `CD${decimalToRoman(num - 400)}`;
    if (num >= 100)
        return `C${decimalToRoman(num - 100)}`;
    if (num >= 90)
        return `XC${decimalToRoman(num - 90)}`;
    if (num >= 50)
        return `L${decimalToRoman(num - 50)}`;
    if (num >= 40)
        return `XL${decimalToRoman(num - 40)}`;
    if (num >= 10)
        return `X${decimalToRoman(num - 10)}`;
    if (num >= 9)
        return `IX${decimalToRoman(num - 9)}`;
    if (num >= 5)
        return `V${decimalToRoman(num - 5)}`;
    if (num >= 4)
        return `IV${decimalToRoman(num - 4)}`;
    if (num >= 1)
        return `I${decimalToRoman(num - 1)}`;
    throw new RangeError();
};
export const codename = (str) => {
