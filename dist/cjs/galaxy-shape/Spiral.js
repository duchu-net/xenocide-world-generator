...
class Spiral {
    ...
    *Generate(random) {
        ...
        try {
            ...
            for (const star of this.GenerateArms(random)) {
                if (star.position.lengthSq() > centralVoidSizeSqr)
                    yield star;
            }
    ...
    GenerateBackgroundStars(random) {
        ...
        return new Sphere_1.Sphere(size, 0.000001, 0.0000001, 0.35, 0.125, 0.35).Generate(random);
    }
    *GenerateArms(random) {
        const { size, swirl, spacing, minimumArms, maximumArms, clusterCountDeviation, clusterCenterDeviation, armClusterScaleDeviation, minArmClusterScale, maxArmClusterScale, } = this.options;
        try {
            const arms = random.integer(minimumArms, maximumArms);
            const armAngle = (Math.PI * 2) / arms;
            const maxClusters = size / spacing / arms;
            for (let arm = 0; arm < arms; arm++) {
                const clusters = parseInt(random.NormallyDistributedSingle(maxClusters * clusterCountDeviation, maxClusters).toFixed());
                for (let i = 0; i < clusters; i++) {
                    ...
                    const center = new three_1.Vector3(0, 0, dist);
                    center.applyAxisAngle(new three_1.Vector3(0, 1, 0), angle);
                    ...
                    const cSize = random.NormallyDistributedSingle4(clsScaleDev, clsScaleMin * 0.5 + clsScaleMax * 0.5, clsScaleMin, clsScaleMax);
                    const densityMean = 0.00025;
                    const stars = new Sphere_1.Sphere(cSize, densityMean, undefined, 1, 1, 1).Generate(random);
                    for (const star of stars) {
                        yield star.Offset(center).swirl(new three_1.Vector3(0, 1, 0), swirl);
                    }
                }
            }
        }
    ...
    *GenerateCenter(random) {
        const { size, swirl, centerClusterDensityDeviation, centerClusterDensityMean, centerClusterCountMean, centerClusterScale, centerClusterCountDeviation, centerClusterPositionDeviation, } = this.options;
        try {
            //Add a single central cluster
            const sphere = new Sphere_1.Sphere(size * centerClusterScale, //size:
            ...
            const cluster = new Cluster_1.Cluster(sphere, centerClusterCountMean, centerClusterCountDeviation, size * centerClusterPositionDeviation, size * centerClusterPositionDeviation, size * centerClusterPositionDeviation);
            for (const star of cluster.Generate(random)) {
                yield star.swirl(new three_1.Vector3(0, 1, 0), swirl * 5);
            }
        }
        catch (err) {
            console.error('!', err);
