...
exports.TerrainSurfaceModificator = void 0;
const three_1 = require("three");
const utils_1 = require("../utils");
const planet_weather_modificator_1 = require("./planet-weather-modificator");
...
function randomUnitVector(random) {
    ...
    return new three_1.Vector3(Math.cos(theta) * sinPhi, Math.sin(theta) * sinPhi, Math.cos(phi));
}
...
class TerrainSurfaceModificator extends surface_modificator_1.SurfaceModificator {
    constructor(options) {
    ...
    random, options) {
        ...
        // 'Generating Weather'
        const weather = new planet_weather_modificator_1.PlanetWeatherGenerator();
    ...
    generatePlanetTectonicPlates(topology, plateCount, oceanicRate, random) {
        ...
        while (plates.length < plateCount && failedCount < 10000) {
            ...
            const plate = new utils_1.Plate(index, new three_1.Color(random.integer(0, 0xffffff)), randomUnitVector(random), random.realInclusive(-Math.PI / 30, Math.PI / 30), random.realInclusive(-Math.PI / 30, Math.PI / 30), oceanic ? random.realInclusive(-0.8, -0.3) : random.realInclusive(0.1, 0.5), oceanic, corner);
            plates.push(plate);
            for (let i = 0; i < corner.tiles.length; ++i) {
...
exports.TerrainSurfaceModificator = TerrainSurfaceModificator;
