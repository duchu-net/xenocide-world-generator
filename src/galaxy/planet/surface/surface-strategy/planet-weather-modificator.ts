...
export class PlanetWeatherGenerator {
  generatePlanetWeather(
    topology: PlanetSurface['topology'],
    partitions: PlanetSurface['partition'],
    heatLevel: number,
    ...
    const { corners: activeMoistureCorners, airMoisture: totalMoisture } = this.initializeAirMoisture(
      topology.corners,
      moistureLevel
    );
  ...
  generateAirCurrentWhorls(planetRadius: number, random: RandomObject) {
    const whorls: Whorl[] = [];
  ...
  calculateAirCurrents(corners: Corner[], whorls: Whorl[], planetRadius: number) {
    let i = 0;
  ...
  initializeAirHeat(corners: Corner[], heatLevel: number) {
    ...
    for (let i = 0; i < corners.length; ++i) {
      ...
      corner.airHeat = (corner.area as number) * heatLevel;
      corner.newAirHeat = 0;
      corner.heat = 0;

      corner.heatAbsorption =
        (0.1 * (corner.area as number)) / Math.max(0.1, Math.min(corner.airCurrentSpeed as number, 1));
      if ((corner.elevation as number) <= 0) {
