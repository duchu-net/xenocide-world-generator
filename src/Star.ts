...
class Star extends CelestialBody {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (stellar_class != null && mass != null) {
          matrice = SPECTRAL_CLASSIFICATION.find(
            (sc) => sc.class == stellar_class.toUpperCase() && mass >= sc.min_sol_mass && mass < sc.max_sol_mass
          );
          if (!matrice) throw { mass: 'not match with stellar classification' };
        } else if (mass != null && stellar_class == null) {
          matrice = SPECTRAL_CLASSIFICATION.find((sc) => mass >= sc.min_sol_mass && mass < sc.max_sol_mass);
        ...
        this.main_sequence_lifetime =
          main_sequence_lifetime != null ? main_sequence_lifetime : this.calcMainSequenceLifetime();
  ...
  calcMainSequenceLifetimeInYears() {
    const main_sequence_lifetime = this.getValues('main_sequence_lifetime').main_sequence_lifetime as number;
    // @ts-ignore
  ...
  generateStellarClassification(args = {}): string {
    // IF MATRICE SET
