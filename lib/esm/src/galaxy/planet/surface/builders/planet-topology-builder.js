...
export class PlanetTopologyBuilder {
    generatePlanetTopology(mesh) {
        const corners = mesh.faces.map((face, index) => new Corner(index, face.centroid.clone().multiplyScalar(1000), face.e.length, face.e.length, face.n.length));
        const borders = mesh.edges.map((edge, index) => new Border(index, 2, 4, 2));
        const tiles = mesh.nodes.map((node, index) => new Tile(index, node.p.clone().multiplyScalar(1000), node.f.length, node.e.length, node.e.length));
