...
exports.PlanetTopologyBuilder = void 0;
const three_1 = require("three");
const utils_1 = require("../utils");
function calculateTriangleArea(pa, pb, pc) {
    const vab = new three_1.Vector3().subVectors(pb, pa);
    const vac = new three_1.Vector3().subVectors(pc, pa);
    const faceNormal = new three_1.Vector3().crossVectors(vab, vac);
    const vabNormal = new three_1.Vector3().crossVectors(faceNormal, vab).normalize();
    const plane = new three_1.Plane().setFromNormalAndCoplanarPoint(vabNormal, pa);
    const height = plane.distanceToPoint(pc);
    const width = vab.length();
    const area = width * height * 0.5;
    return area;
}
class PlanetTopologyBuilder {
    generatePlanetTopology(mesh) {
        const corners = mesh.faces.map((face, index) => new utils_1.Corner(index, face.centroid.clone().multiplyScalar(1000), face.e.length, face.e.length, face.n.length));
        const borders = mesh.edges.map((edge, index) => new utils_1.Border(index, 2, 4, 2));
        const tiles = mesh.nodes.map((node, index) => new utils_1.Tile(index, node.p.clone().multiplyScalar(1000), node.f.length, node.e.length, node.e.length));
        ...
        borders.forEach((border, index) => {
            ...
            const averageCorner = new three_1.Vector3(0, 0, 0);
            let n = 0;
        ...
        tiles.forEach((tile, index) => {
            ...
            tile.averagePosition = new three_1.Vector3(0, 0, 0);
            ...
            tile.boundingSphere = new three_1.Sphere(tile.averagePosition, maxDistanceToCorner);
        });
...
exports.PlanetTopologyBuilder = PlanetTopologyBuilder;
//# sourceMappingURL=planet-topology-builder.js.map