...
exports.PlanetMeshBuilder = void 0;
const three_1 = require("three");
const icosahedron_builder_1 = require("./icosahedron-builder");
...
class PlanetMeshBuilder {
    ...
    //   return { ...this.model };
    // }
    generatePlanetMesh(icosahedronSubdivision, topologyDistortionRate, random) {
        const mesh = icosahedron_builder_1.IcosahedronBuilder.generateSubdividedIcosahedron(icosahedronSubdivision);
        this.mesh = mesh;
    ...
    static findNextFaceIndex(mesh, nodeIndex, faceIndex) {
    ...
    static getEdgeOppositeFaceIndex(edge, faceIndex) {
    ...
    static distortMesh(mesh, degree, random) {
        ...
        const rotationPredicate = function (oldNode0, oldNode1, newNode0, newNode1) {
    ...
    static conditionalRotateEdge(mesh, edgeIndex, predicate) {
    ...
    static getFaceOppositeNodeIndex(face, edge) {
    ...
    static relaxMesh(mesh, multiplier) {
        const totalSurfaceArea = 4 * Math.PI;
        const idealFaceArea = totalSurfaceArea / mesh.faces.length;
        const idealEdgeLength = Math.sqrt((idealFaceArea * 4) / Math.sqrt(3));
        const idealDistanceToCentroid = ((idealEdgeLength * Math.sqrt(3)) / 3) * 0.9;
        const pointShifts = mesh.nodes.map((node) => new three_1.Vector3(0, 0, 0));
        mesh.faces.forEach((face) => {
        ...
        mesh.nodes.forEach((node, index) => {
            ...
            pointShifts[index] = node.p.clone().add(plane.projectPoint(pointShifts[index], new three_1.Vector3())).normalize();
        });
    ...
    static calculateFaceCentroid(pa, pb, pc) {
...
exports.PlanetMeshBuilder = PlanetMeshBuilder;
//# sourceMappingURL=planet-mesh-builder.js.map